...
I want to especially highlight Diomidis Spinellis' wonderful github repository, which has an even more extensive list [[1]](https://github.com/dspinellis/latex-advice?tab=readme-ov-file).  
...
- `\slash` instead of `/` allows a line break.
...
- If there is a capital letter preceding a sentence-ending period you have to add an `\@` so that LaTeX does not treat it as an abbreviation. Example: `...Pentium III\@. This...`. Regex search for `[A-Z]\.`[[5]](https://john.regehr.org/latex/).
...
- Use `Section~\ref{sec:...}` or `Sect.~\ref{sec:...}` depending on the journal.
...
- A useful list of mathematical symbols can be found [here](  
  https://www.cmor-faculty.rice.edu/~heinken/latex/symbols.pdf).
- Put numbers that are part of math/science and not part of the English language in math mode. Or how [[6]](https://www.economics.utoronto.ca/osborne/latex/LTXERR.HTM) puts it:
...
- In strict scientific typesetting, the "d" in an integral or the "e" in an exponential should be upright, not italicized, because they are operators/constants, not variables. Example: `\int_0^\infty \mathrm{e}^{-x}\,\mathrm{d}x` [[7]](https://en.wikibooks.org/wiki/LaTeX/Mathematics).
- If you have a multi-letter variable you should use `$T_{\mathrm{in}}$` and **not** write `$T_{in}$`

## Units

- Coordinates need a degree sign and a space when naming the direction (e.g. `30\textdegree~N`).
...
- Units must be written exponentially (e.g. W m⁻²
).
...
- Journals have different guidelines for units. Some have their own command; otherwise use siunitx if additional packages are allowed. There you can write 10 km h⁻¹
 via `\qty{10}{\kilo\metre\per\hour}`.
- SI base units and SI-accepted units must be abbreviated in conjunction with numbers (e.g. the velocity is 10 km h⁻¹
) and must be written out without numbers (e.g. the velocity is given in kilometres per hour).
...
- Make sure the figures are colour-blind friendly. You can check them, for example, at [https://www.color-blindness.com/coblis-color-blindness-simulator/](https://www.color-blindness.com/coblis-color-blindness-simulator/).
...
- An empty line after or before a float environment begins a new paragraph, so avoid this unless intended: 
    ```latex
    Text here.
    \begin{figure}[!htbp]
    \centering
...
- Use `date` (YYYY-MM-DD) if you are using BibLaTeX; stick to `year` if the journal requires BibTeX, as `date` is not backward compatible.
- Make sure the author list does **not** contain "et al.", "and others", or similar.
- Use `url` and `urldate` to record when a website was accessed, **not** the `note` field, unless your bibliography style doesn't support it.
...
- Make use of the `howpublished` field for `@misc` entries to address source types not directly supported by BibTeX. This field is particularly useful when citing web pages if your bibliography style is old and does not support the `url` field [[4]](https://bibtex.eu/fields/howpublished/).

## Resources

[1] [https://github.com/dspinellis/latex-advice?tab=readme-ov-file](https://github.com/dspinellis/latex-advice?tab=readme-ov-file)

[2] [https://github.com/cxli233/FriendsDontLetFriends](https://github.com/cxli233/FriendsDontLetFriends)

[3] [https://web.evanchen.cc/latex-style-guide.html](https://web.evanchen.cc/latex-style-guide.html)

[4] [https://bibtex.eu/fields/howpublished/](https://bibtex.eu/fields/howpublished/)

[5] [https://john.regehr.org/latex/](https://john.regehr.org/latex/)

[6] [https://www.economics.utoronto.ca/osborne/latex/LTXERR.HTM](https://www.economics.utoronto.ca/osborne/latex/LTXERR.HTM)

[7] [https://en.wikibooks.org/wiki/LaTeX/Mathematics](https://en.wikibooks.org/wiki/LaTeX/Mathematics)